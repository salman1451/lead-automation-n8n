...
Every node was executed and verified individually before running full
end-to-end paths, including:
- Happy path (qualified, high-priority lead) — CRM record created, log
  written, personalized email sent, Slack notification posted.
- Invalid payload (missing required fields) → 400 response, error logged.
- Duplicate resubmission (same email + message) → blocked, no reprocessing.
- Spam-flagged lead → business-rule override correctly forces
  `priority: low`, `is_relevant: false`, no email/notification sent.
- AI-call-failure branch (Groq credential/URL deliberately broken for the
  test) → rule-based fallback fired correctly, failure logged.
- Email-generation-failure branch (same deliberate-break method) →
  `Log Email Failure` fired without blocking the rest of the pipeline.
- Global Error Handler workflow — confirmed it fires on an unhandled node
  failure (Airtable base temporarily pointed at an invalid ID) and writes
  to the `Logs` table independently of the main workflow's own error paths.

## Known trade-offs / things to be upfront about in the interview

- **No webhook authentication** — a deliberate scope call for this
  assessment; a real production deployment should add a shared-secret
  header check before the validation step.
- **ngrok tunnel, not a permanent host** — used to expose the local n8n
  ...
  randomly generated per session and not stable across restarts; a real
  deployment would run n8n Cloud or self-hosted behind a fixed domain.
- **`Airtable - Upsert Lead` operation is hardcoded to Create** — the
  cleanest real fix is two separate Create/Update nodes gated by an IF on
  `existingRecordId`, since the Airtable node's Operation parameter doesn't
  reliably take a per-item expression in this n8n version. Works correctly
  for new leads as submitted; returning-lead updates would need that follow-up.
- **Groq's `openai/gpt-oss-120b`** is a reasoning model — it spends a
  meaningful chunk of tokens "thinking" even for simple tasks like writing a
  short email. Fine for this assessment; a non-reasoning model would be a
  better cost/latency fit for the email-generation step specifically in a
  real deployment.
- **Submission throttling vs. true rate limiting** — the 5/hour/email cap is
  a business-protection rule, not upstream API rate-limit handling; the
  latter is covered by retry/backoff on the external call nodes instead.
